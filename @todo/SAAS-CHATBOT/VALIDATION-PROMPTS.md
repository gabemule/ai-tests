...
> (`model-benchmark.md`) and self-host breakeven (`infra.md`). The live price per token now lives in
> `PRICING/openrouter-pricing.md` (the SSOT, moved out of `ANALYSIS/` during the split). This prompt
> validates the source and the **source→derived coherence**.
>
> **Note:** the SSOT for prices is **`PRICING/openrouter-pricing.md`**, regenerated by
> `PRICING/extract/fetch-openrouter-pricing.sh` and inspected via `PRICING/extract/openrouter-pricing.html`.
...
2. @todo/SAAS-CHATBOT/PRICING/openrouter-pricing.md  — price per token (SSOT; moved here from ANALYSIS/)
...
- Run `bash @todo/SAAS-CHATBOT/PRICING/extract/fetch-openrouter-pricing.sh` to regenerate the live
  price data feeding `openrouter-pricing.md`.
- Open `@todo/SAAS-CHATBOT/PRICING/extract/openrouter-pricing.html` and review the **Newest** tab
  (NEW/PROMO badges, new price buckets). Any model surfaced there that lacks a quality score in
  `ANALYSIS/model-benchmark.md` (e.g. Qwen3.7 Plus/Max marked "n/d") must be **benchmarked and added
  to `model-benchmark.md`** — that's how the factual base stays current so PRICING/ can derive from it.
