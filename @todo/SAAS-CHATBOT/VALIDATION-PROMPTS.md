...
> **(3)** factual base in `ANALYSIS/` ↔ `PRICING/`.
...
- @todo/SAAS-CHATBOT/ANALYSIS/*.md           (factual base the prices derive from)
...
- UNSOURCED — a PRICING number with no backing in ANALYSIS/ (or that diverges from it).
...
M3. Anchor & model prices — every model price in models.md matches ANALYSIS/ (and openrouter SSOT)?
...
3. Source→number table: each key PRICING number → ANALYSIS source (✅ matches / ❌ diverges → correct).
...
agree with each other and with their source.
```

---

## 📊 PROMPT 3 — Cross-check the factual base (`ANALYSIS/` ↔ `PRICING/`)

> SSOT for prices = **`PRICING/openrouter-pricing.md`**, regenerated by
> `PRICING/extract/fetch-openrouter-pricing.sh`, inspected via `PRICING/extract/openrouter-pricing.html`.
...
- Open `@todo/SAAS-CHATBOT/PRICING/extract/openrouter-pricing.html` → "Newest" tab. Any model there
  used by PRICING but lacking a score in ANALYSIS/model-benchmark.md is a MISSING finding.

READ:
- @todo/SAAS-CHATBOT/ANALYSIS/model-benchmark.md     (quality scores)
- @todo/SAAS-CHATBOT/PRICING/openrouter-pricing.md   (price per token — SSOT)
- @todo/SAAS-CHATBOT/ANALYSIS/infra.md               (self-host breakeven)
...
D8. Coverage — every model PRICING uses has a benchmark score in ANALYSIS/?
...
from ASSUMPTION, flag every PRICING number with no traceable ANALYSIS/ backing. No architecture (P1),
no pricing redesign (P2).
```
