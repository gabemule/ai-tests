...
a tenant where we earn **$0 on tokens** *and* who can deduce our margin (`PRICING/README.md` §1.4).
...
- **Managed is the default/recommended mode on every tier — including Free.** It's where the routing
  ...
  small included starter balance + hard wallet cap (`PRICING/plans.md` §6.2).
- **BYOK is not a self-serve option** on the paid ladder — it's the **Enterprise-only paid add-on**
  (ADR 009; `PRICING/billing.md` §4.3), sold on governance, never as cost-saving.
