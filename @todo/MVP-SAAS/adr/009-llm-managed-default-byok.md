...
- **Sequencing (feature-graph):** BYOK ships first inside `chat-sse` as the technical bootstrap (zero
  financial risk, no wallet yet). **Managed** (`managed-mode`) lands later, on a base de-risked by
  `metering` (shadow) + `retrieval-eval`, and becomes the default; from then, BYOK is offered only as
